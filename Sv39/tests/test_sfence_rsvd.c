...
/**
 * test_smode_sfence_perm_change - Change PTE from RW to R-only, sfence, store
 *
 * arg = packed struct: low 48 bits = test VA, high 16 bits unused
 * We pass the PTE address via a global since S-mode can access it
...
bool test_sv39_rsvd02(void) {
     ...
     * Note: bits 61:60 may be used by Svpbmt extension (PBMT field),
     * so we use bit 56 which is always reserved. */
    uintptr_t pt_page = get_pt_page_addr(&ctx, test_va, 0);
    TEST_ASSERT("found L0 page table", pt_page != 0);
