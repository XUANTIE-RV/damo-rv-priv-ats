...
/*
 * test_satp.c - Group 11: satp CSR Control (SATP-01/04/07/09)
 *
 * Tests:
 *   SATP-01: MODE=Bare disables VM
 *   SATP-04: MODE=Sv57 enables VM
 *   SATP-07: ASID basic functionality
 *   SATP-09: Unsupported MODE value (WARL behavior)
 *
 * Note: Sv57 is the highest supported mode, so there is no
 * mode switch test (no higher mode to switch to).
 */

TEST_REGISTER(test_sv57_satp01);
bool test_sv57_satp01(void) {
...
TEST_REGISTER(test_sv57_satp04);
bool test_sv57_satp04(void) {
    TEST_BEGIN("SATP-04: MODE=Sv57 enables VM");
    ...
    TEST_ASSERT("Sv57 VM enabled, read/write succeeds", result == 0);

    pt_pool_reset();
    TEST_END();
}

TEST_REGISTER(test_sv57_satp07);
bool test_sv57_satp07(void) {
    ...
    uintptr_t satp_val = MAKE_SATP(SATP_MODE_SV57, test_asid, root_ppn);
    CSRW(satp, satp_val);
    vm_sfence_vma(0, 0);
...
TEST_REGISTER(test_sv57_satp09);
bool test_sv57_satp09(void) {
