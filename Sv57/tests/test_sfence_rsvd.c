...
TEST_REGISTER(test_sv57_sfence01);
bool test_sv57_sfence01(void) {
...
TEST_REGISTER(test_sv57_sfence04);
bool test_sv57_sfence04(void) {
...
TEST_REGISTER(test_sv57_sfence05);
bool test_sv57_sfence05(void) {
...
TEST_REGISTER(test_sv57_rsvd01);
bool test_sv57_rsvd01(void) {
...
TEST_REGISTER(test_sv57_rsvd02);
bool test_sv57_rsvd02(void) {
...
TEST_REGISTER(test_sv57_nlpte04);
bool test_sv57_nlpte04(void) {
     ...
     * at L4, L3, L2, and L1 should have D=0, A=0, U=0 by default
     * (only PTE_V set). The leaf PTE at L0 has full permissions.
     *
     * This test verifies that non-leaf PTEs with D=A=U=0 do not cause
     * page faults during the 5-level walk.
     */
    ...
    /* Verify non-leaf PTEs at L4 (root) have D=A=U=0 */
    uintptr_t test_va = (uintptr_t)test_data_area;

    /* Check L4 (root) PTE - Sv57 root is at level 4 */
    uintptr_t *root = ctx.root_pt;
    uintptr_t vpn4 = VA_VPN4(test_va);
    uintptr_t l4_pte = root[vpn4];
    TEST_ASSERT("L4 non-leaf PTE is valid", (l4_pte & PTE_V) != 0);
    TEST_ASSERT("L4 non-leaf PTE is non-leaf",
                (l4_pte & (PTE_R | PTE_W | PTE_X)) == 0);
    TEST_ASSERT("L4 non-leaf PTE has D=A=U=0",
                (l4_pte & (PTE_D | PTE_A | PTE_U)) == 0);
